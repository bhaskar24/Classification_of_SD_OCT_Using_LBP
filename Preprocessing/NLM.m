function [sig_est] = NLM(input)
%===================================================================================
%   nlmeans_lnla2008_goossens.m
%   A 2-d version of the NLMeans filter from our paper
%
%   [1] B. Goossens, H.Q. Luong, A. Pizurica, W. Philips, "An improved non-local 
%       means algorithm for image denoising," in 2008 International Workshop on 
%       Local and Non-Local Approximation in Image Processing (LNLA2008), 
%       Lausanne, Switzerland, Aug. 25-29 (invited paper)
%
%   The implementation is here for images. Note that the original version
%   was written in C++ because of computation time. Later, the C++ version
%   was ported to MATLAB (on request of other researchers). This MATLAB version 
%   is solely meant for educational purposes.
%===================================================================================
%   May 2008-June 2010  Bart Goossens, Ghent University
%   Department of Telecommunications and Information Processing
%   UGent-TELIN-IPI-IBBT
%   bart.goossens (at) telin.ugent.be   
%===================================================================================
sig_orig = double(input); % original test image
[M,N]=size(sig_orig);
...
%===================================================================================
sigma = 25;               % noise standard deviation for the simulation
correlated_noise = 0;     % 0 - white Gaussian noise
                          % 1 - correlated Gaussian noise                                                   

W = 7;                    % half window size (actual window size = 2*W+1) 
...
wnd_size = 11;             % half size of the _search_ window
...
%figure,
%subplot(121),imshow(uint8(sig),[]),title(sprintf('Noisy image PSNR=%f dB', GetPSNR(sig,sig_orig)));
%subplot(122),imshow(uint8(sig_est),[]),title(sprintf('Denoised image PSNR=%f dB', GetPSNR(sig_est,sig_orig)));
sig_est = uint8(sig_est);
end